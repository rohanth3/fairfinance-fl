...
<style>
    * {
    ...
    .stMetricCard { 
        background: linear-gradient(135deg, #00509e 0%, #007acc 100%) !important;
    ...
    .metric-card {
        ...
        padding: 25px;
        text-align: center;
        box-shadow: 0 10px 25px rgba(102, 163, 255, 0.15);
    }
...
def render_page_content(selected):
    ...
    # Display global metrics at the top of the main area instead of sidebar
    col_a, col_b, col_c = st.columns(3)
    col_a.metric("Current Round", total_rounds)
    col_b.metric("Active Clients", active_clients)
    col_c.metric("Data Points", total_rounds)
    st.markdown("---")
    ...
    if selected == "Dashboard":
        ...
        if not clients_df.empty:
            ...
            with col1:
                # Pie chart for rounds distribution
                if 'Rounds Completed' in clients_df.columns:
            ...
            with col2:
                ...
                if 'Avg Accuracy' in clients_df.columns:
            ...
            st.info("ℹ️ No client data available yet. Waiting for clients to connect and complete their first training round...")
