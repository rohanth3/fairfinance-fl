...
class SimpleFederatedServer:
    ...
    def handle_client(self, client_socket):
        ...
        try:
            ...
            client_socket.settimeout(1200)  # 1200 second timeout for recv
        ...
        try:
            while True:
                ...
                if msg_type == 'model_update':
                    ...
                    # Check condition before lock
                    if len(active_clients) >= 1 and round_num > self.last_aggregated_round:
                        models_to_aggregate = None
                        with self.round_models_lock:
                            # Double-check inside lock to prevent race conditions
                            if round_num > self.last_aggregated_round:
                                models_to_aggregate = self.round_models.copy()
                                # Mark this round as aggregated immediately to prevent other threads from aggregating
                        ...
                        if models_to_aggregate:
                            try:
                                ...
                                client_metrics = {}
                                for cid in models_to_aggregate.keys():
                                    model_data = models_to_aggregate[cid]
                                    client_metrics[cid] = {
                                        'accuracy': model_data['accuracy'],
                                        'data_quality': model_data['data_quality']
                                    }
                                
                                # Perform aggregation (HEAVY MATH - now outside the lock!)
                                agg_result = self.aggregator.aggregate_models(
                                    {cid: data['params'] for cid, data in models_to_aggregate.items()},
                                    client_metrics
                                )
                                ...
                                print(f"[SERVER] Aggregated {len(models_to_aggregate)} models (Federated Round {self.current_round}), Quality: {agg_result.quality_score:.4f}")
                                ...
                                # Revert last aggregated round on failure so another thread might try
                                with self.round_models_lock:
                                    if self.last_aggregated_round == round_num:
                                        self.last_aggregated_round = round_num - 1
