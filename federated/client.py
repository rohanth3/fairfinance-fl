...
from federated.differential_privacy import DifferentialPrivacyMechanism, create_dp_config
...
def train(model, loader, learning_rate=0.001, batch_size=32, local_epochs=1):
    ...
    criterion = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([fraud_weight]).to(device))
    ...
    for epoch in range(local_epochs):
        ...
        for batch_idx, (X, y) in enumerate(loader):
            X, y = X.to(device), y.to(device).unsqueeze(1)  # Add dimension for BCEWithLogitsLoss
            optimizer.zero_grad()
    ...
    # Ensure state dict is moved to CPU before returning so pickling doesn't fail
...
def main():
    ...
    try:
        ...
        try:
            dp_config = create_dp_config(privacy_level="moderate")
            dp_config.enable = True # Enable privacy for Phase 3
            dp_mechanism = DifferentialPrivacyMechanism(config=dp_config)
            print("[CLIENT] ✓ Differential Privacy initialized")
        except Exception as e:
            print(f"[CLIENT] Warning: Could not initialize DP: {e}")
            dp_mechanism = None
            ...
            try:
                ...
                try:
                    ...
                    if dp_mechanism is not None:
                        # Apply DP (happens efficiently with PyTorch tensors)
                        model_params = dp_mechanism.privatize_update(model_params, strategy="laplace")
                        # Ensure parameters are back on CPU for socket pickling
                        model_params = {k: v.cpu() if hasattr(v, 'cpu') else v for k, v in model_params.items()}
                        
                except Exception as e:
                    print(f"[CLIENT] Error during training/DP: {e}")
                    continue
